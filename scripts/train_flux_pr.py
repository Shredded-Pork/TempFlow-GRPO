...
from flow_grpo.diffusers_patch.flux_pipeline_with_logprob_perstep import pipeline_with_logprob as pipeline_with_logprob_perstep
...
def main(_):
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for i in tqdm(
            ...
            with autocast():
                with torch.no_grad():
                    images, latents, sde_latents, image_ids, text_ids, log_probs = pipeline_with_logprob_perstep(
                        pipeline,
                        prompt_embeds=prompt_embeds,
                        pooled_prompt_embeds=pooled_prompt_embeds,
            ...
            samples.append(
                {
                    ...
                    "image_ids": image_ids.unsqueeze(0).repeat(latents.shape[0],1,1),
                    "timesteps": timesteps,
                    "latents": latents[
                        :, :-1
                    ],  # each entry is the latent before timestep t
                    "next_latents": sde_latents,  # each entry is the latent after timestep t
                    "log_probs": log_probs,
                    "rewards": rewards,
                }
            )
        ...
        for sample in tqdm(
            ...
            for item in sample['rewards']:
                ...
                for key, value in rewards.items():
                    ...
                    if config.prompt_fn == "geneval" or config.reward_fn.keys() == ["hpsv3"]:
        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            # train
            pipeline.transformer.enable_gradient_checkpointing() # train 1024 resolution
