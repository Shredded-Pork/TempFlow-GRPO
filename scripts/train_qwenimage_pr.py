...
def compute_log_prob(transformer, pipeline, sample, j, config, rank):
    img_shapes = [[(1, config.resolution // pipeline.vae_scale_factor // 2, config.resolution // pipeline.vae_scale_factor // 2)]]
    ...
    # txt_seq_lens/,sample["prompt_embeds_mask"]�sample["prompt_embeds"]��	�Ł�padding
    ...
    noise_pred = transformer(
        hidden_states=torch.cat([sample["latents"][0, j].unsqueeze(0), sample["latents"][0, j].unsqueeze(0)], dim=0),
        timestep=torch.cat([sample["timesteps"][0, j].unsqueeze(0), sample["timesteps"][0, j].unsqueeze(0)], dim=0) / 1000,
        guidance=None,
        encoder_hidden_states_mask=torch.cat([sample["prompt_embeds_mask"][0].unsqueeze(0), sample["negative_prompt_embeds_mask"][0].unsqueeze(0)], dim=0),
        encoder_hidden_states=torch.cat([sample["prompt_embeds"][0].unsqueeze(0), sample["negative_prompt_embeds"][0].unsqueeze(0)], dim=0),
        img_shapes=img_shapes*2,
        txt_seq_lens=[txt_seq_lens[0]]+[negative_txt_seq_lens[0]],
    )[0]
...
def main(_):
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for i in tqdm(
            ...
            with autocast():
                with torch.no_grad():
                    images, latents, sde_latents, log_probs, timesteps, prompt_embeds, negative_prompt_embeds, prompt_embeds_mask, negative_prompt_embeds_mask = pipeline_with_logprob_perstep(
                        pipeline,
                        prompts,
                        negative_prompt=[" "]*len(prompts),
            ...
            latents = torch.stack(
                latents, dim=1
            ).repeat_interleave(exploration_k, dim=0)  # (batch_size, num_steps + 1, seq_len, channels)
            ...
            # Recreate timesteps similar to the pipeline
            timesteps = torch.stack(timesteps).unsqueeze(0).repeat(
            ...
            samples.append(
                {
                    ...
                    "next_latents": sde_latents,  # each entry is the latent after timestep t (SDE version)
                    "log_probs": log_probs,
                    "rewards": rewards,
                }
            )
        ...
        # gathered_rewards = {key: gather_tensor(value, world_size) for key, value in samples["rewards"].items()} # prompt std
        gathered_rewards = samples["rewards"] # seed std
        ...
        if config.per_prompt_stat_tracking:
            ...
            # prompt_ids = gather_tensor(samples["prompt_ids"], world_size).cpu().float().numpy() # prompt std
            prompt_ids = samples["prompt_ids"].cpu().numpy() # seed std
        ...
        advantages = gather_tensor(advantages.to(device), world_size)  # seed std
        samples["advantages"] = (
        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    unclipped_loss = -1.53 * std_dev_t * advantages * ratio
                    clipped_loss = -1.53 * std_dev_t * advantages * torch.clamp(
                        ratio,
