""" This repo for process reward in SD3 """
...
class DistributedKRepeatSampler(Sampler):
    def __init__(self, dataset, batch_size, k, num_replicas, rank, seed=0):
        ...
        self.batch_size = batch_size  # batch of each gpus
...
def calculate_zero_std_ratio(prompts, gathered_rewards):
    """
    Calculates the proportion of rewards for each unique prompt whose standard deviation is zero.

    Parameters:
        prompts: List of prompts
        gathered_rewards: Dictionary containing reward values, must include the key 'ori_avg'

    Returns:
        zero_std_ratio: The proportion of rewards for which the standard deviation is zero.
        prompt_std_devs: An array of standard deviations for each unique prompt.

    """
   # turn prompt into numpy array
    ...
    # Get the reward value corresponding to each prompt word in groups
    ...
    # Calculate the proportion of zero standard deviation
...
def main(_): 
    ...
    if config.prompt_fn == "general_ocr":
        ...
        # craete a loop DataLoader
        train_sampler = DistributedKRepeatSampler( 
            dataset=train_dataset,
            batch_size=config.sample.train_batch_size,
            k=config.sample.num_image_per_prompt,  # k
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for i in tqdm(
            ...
            latents = torch.stack(
                latents, dim=1
            ).repeat_interleave(exploration_k, dim=0)  # (batch_size, num_steps + 1, 16, 96, 96)
            sde_latents = torch.stack(sde_latents, dim=1)
            log_probs = torch.stack(log_probs, dim=1)  # shape after stack (batch_size, num_steps)
            kls = torch.stack(kls, dim=1).repeat_interleave(exploration_k, dim=0)
            kl = kls.detach()
            ...
            timesteps = pipeline.scheduler.timesteps.repeat(
                config.sample.train_batch_size*exploration_k, 1
            ...
            samples.append(
                {
                    ...
                    "pooled_prompt_embeds": pooled_prompt_embeds.repeat_interleave(exploration_k, dim=0),
                    "timesteps": timesteps,
                    "latents": latents[
                        :, :-1
                    ],  # each entry is the latent before timestep t
                    "next_latents": sde_latents,
                    "log_probs": log_probs,
                    "kl": kl,
                    "rewards": rewards,
                }
        ...
        for sample in tqdm(
            ...
            tmp_rewards = {}
            for item in sample['rewards']:
                rewards, reward_metadata = item.result()
                for key, value in rewards.items():
                    if key not in tmp_rewards.keys(): tmp_rewards[key] = []
                    if config.prompt_fn == "geneval":
                        tmp_rewards[key].append(torch.as_tensor(value, device=accelerator.device).float())
                    else:
                        if isinstance(value, list): 
                            value = torch.stack(value, dim=0)
                        tmp_rewards[key].append(value)
            # process different key
        ...
        samples["rewards"]["avg"] = samples["rewards"]["avg"] - config.sample.kl_reward*samples["kl"]
        ...
        if config.per_prompt_stat_tracking:
            ...
            prompt_ids = samples["prompt_ids"].cpu().numpy()
            ...
            accelerator.log(
                {
                    ...
                    "trained_prompt_num": trained_prompt_num*config.sample.num_image_per_prompt,
        ...
        advantages = accelerator.gather(advantages.to(accelerator.device))
        samples["advantages"] = (
        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            # train            
            # pipeline.transformer.enable_gradient_checkpointing() # 训练1024分辨率
            ...
            for i, sample in tqdm(
                ...
                if config.train.cfg:
                    ...
                    embeds = torch.cat(
                        [train_neg_prompt_embeds[:len(sample["prompt_embeds"])].repeat_interleave(sample["prompt_embeds"].shape[0],dim=0), sample["prompt_embeds"]]
                    )
                    pooled_embeds = torch.cat(
                        [train_neg_pooled_prompt_embeds[:len(sample["pooled_prompt_embeds"])].repeat_interleave(sample["pooled_prompt_embeds"].shape[0],dim=0), sample["pooled_prompt_embeds"]]
                    )
                else:
